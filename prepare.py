...
(function () {
  ...
  if (result.ok) {
    ...
    status.textContent = '这个浏览器或网页模式暂时不能使用。请用更新后的 Safari、Edge、Chrome 或 Firefox 普通窗口打开本页；不要使用 IE 兼容模式。';
...
<p><strong>采集按钮版本：{revision}</strong> · 已安装过旧书签时，编辑旧书签，将网址替换为本页下方的完整内容。刷新本页后再复制。</p>
<ol><li>在平时登录教务的浏览器中打开本页，将下方按钮拖到顶部的书签或收藏夹栏。<br><small>Edge：收藏夹栏 · Chrome：书签栏 · Firefox：书签工具栏。按 {bookmark_shortcut()} 显示。Safari 请选“显示 → 显示个人收藏栏”，再把按钮拖入该栏。请在同一个浏览器完成添加和学校登录。</small></li>
<li>双击本地 <code>同步课表.cmd</code>，它会等待浏览器下载的课表。</li>
<li>在这个浏览器的 <a href="https://jwstu.shsmu.edu.cn/Home" target="_blank" rel="noopener">教务首页</a>正常登录。Safari 首次使用时，先在首页打开一次“我的课表”，确认能看到课程，再回到显示本人学号的首页点击课表按钮。等待页面提示采集完成，本地窗口会自动生成结果。</li></ol>
...
<p>Safari：按住 Control 点按旧课表书签，选择“编辑地址”，粘贴上方完整网址。也可在“书签 → 编辑书签”中编辑地址。请保留开头的 <code>javascript:</code>；不要直接粘贴到智能搜索栏执行。</p>
<p><small>书签仅向学校发送正常的同源读取请求；认证由现有浏览器处理。下载只包含课表响应，不读取或导出 Cookie、密码、会话存储。不安装或更改扩展，不启动或重启浏览器。生成的 JSON 请保存在同步窗口显示的下载目录。</small></p></main></html>'''
    if desktop:
        page = desktop_bookmark_page(config, bookmark, display, revision)
...
def desktop_bookmark_page(config, bookmark, display, revision):
...
<ol><li>按 <b>{bookmark_shortcut()}</b> 显示顶部的书签或收藏夹栏。<br><small>Edge：收藏夹栏 · Chrome：书签栏 · Firefox：书签工具栏。<br><b>Safari：选“显示 → 显示个人收藏栏”</b>，按钮应放在地址栏下方的个人收藏栏。</small></li>
...
<p class="hint"><b>Safari 首次采集：先打开一次学校的“我的课表”。</b><br>在正常登录的教务首页点击“我的课表”，确认课程已显示，再回到显示本人学号的首页点击课表书签。仅登录首页时，学校可能尚未准备好课表数据；若提示“缺少 List 数组”，也先完成这一步再试。</p>
...
<p>Safari：按住 Control 点按旧课表书签 →“编辑地址”，或在“书签 → 编辑书签”中编辑地址。保留完整的 <code>javascript:</code> 开头；不要粘贴到智能搜索栏执行，也不要只收藏本安装页。</p>
...
<p class="hint">Safari 采集后请查看工具栏的下载列表。若没有 <code>shsmu-capture-…json</code>，在教务页点“重新下载采集文件”；这只保存刚才的结果，不会再读取学校。已下载到其他文件夹时，在助手点“文件已经下载”选择它。</p>
<p><small>学期：{html.escape(config['semester'])} · 采集按钮版本：{revision}<br>
