...
当前安装页为 **修正版 6**，增加下载恢复、明确的错误提示和其他使用者的入口修补；完整场景和恢复步骤见 [使用体验与恢复](USABILITY.md)。已安装书签需要手动替换网址。

## 安装

1. 准备 Windows、Python 3.12+（`py` 启动器或 PATH 中可用的 `python`）和 Chrome。
...
若 Chrome 使用自定义下载目录，在 `config.local.json` 添加 `downloads_dir`（例如 `D:/Downloads`），或运行：

```powershell
.\.venv\Scripts\python.exe -X utf8 sync.py --downloads "你的下载目录"
```

配置支持 UTF-8 BOM、环境变量和 `~`；相对下载路径按配置文件所在目录解析。等待器识别新文件及同名覆盖，等文件大小和修改时间稳定后读取；收到失败诊断时提示查看 Chrome，继续等完整文件。
