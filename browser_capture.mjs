...
export async function collectSchedule(config, io) {
  const fail = message => { throw Object.assign(new Error(message), {code:'DATA_VALIDATION'}); };
  ...
  function validDay(day) {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(day) || !Number.isFinite(parsed.getTime()) || parsed.toISOString().slice(0,10)!==day) fail('日期无效，请核对本机配置并重新生成书签');
    return day;
  }
